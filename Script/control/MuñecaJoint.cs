...
public class MuñecaJoint : MonoBehaviour
{
    HingeJoint hinge;
    Rigidbody rb;
    ...
    void Start()
    {
        boton = GameObject.Find("Cubo.005");
    ...
    void Update()
    {
        float angle = GameObject.Find("Mano360").GetComponent<Transform>().localEulerAngles.z;
        if (angle > 180)
        {
            angle = angle - 360;
        }
        //Debug.Log(angle);
        if (Mathf.Round(angle) >= 180)
        {
            Debug.Log("Limite positivo");
